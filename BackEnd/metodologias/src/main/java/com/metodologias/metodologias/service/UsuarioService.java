...
public class UsuarioService {
	...
	public UsuarioDTO autenticarUsuario(UsuarioShortDTO usuario) throws UsuarioUnAuthorized,UsuarioNotFound{
		Optional<Usuario> _usuario = usuarioRepository.findByEmail(usuario.getEmail());
		if (_usuario.isPresent()) {
			if (_usuario.get().getContrasena().equals(usuario.getContrasena())){
				return this.convertToUsuarioDTO(_usuario.get());
			}else {
				throw new UsuarioUnAuthorized();
			}
		}else {
