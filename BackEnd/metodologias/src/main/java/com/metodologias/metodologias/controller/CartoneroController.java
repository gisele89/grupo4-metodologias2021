...
public class CartoneroController {
	...
	public @ResponseBody ResponseEntity<CartoneroDTO> editCartonero(@PathVariable("id") Long idCartonero,@RequestBody CartoneroDTO cartonero ){
		try {	
			return new ResponseEntity<CartoneroDTO>(cartoneroService.editCartonero(idCartonero,cartonero),HttpStatus.OK);
		}catch(CartoneroNotFound e) {
			LOGGER.error(INTERNAL_SERVER_ERROR,e);
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		}catch(Exception e) {
